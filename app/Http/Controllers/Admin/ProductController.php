...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // $products = Product::with('variables')->get();

        // foreach ($products as $product) {
        //     if ($product->new_price > 0) {
        //         // Calculate wholesale price: 20% less than new price
        //         $wholesalePrice = $product->new_price * 0.80;

        //         // Update the product
        //         $product->wholesale_price = $wholesalePrice;
        //         $product->save();

        //         // Update all product variables with same ratio (20% less)
        //         foreach ($product->variables as $variable) {
        //             $variable->wholesale_price = $variable->new_price * 0.80;
        //             $variable->save();
        //         }
        //     }
        // }
    ...
    public function store(Request $request)
    {
        ...
        $input = $request->except(['image', 'product_type', 'files', 'sizes', 'colors', 'purchase_prices', 'old_prices', 'new_prices', 'wholesale_prices', 'stocks', 'images', 'pro_barcodes']);
        ...
        if ($request->type == 0) {
            ...
            $input['wholesale_price'] = $request->wholesale_prices[0];
            $input['stock'] = 0;
        } else {
        ...
        if ($request->stocks) {
            ...
            if (is_array($stocks)) {
                foreach ($stocks as $key => $stock) {
                    ...
                    $variable->wholesale_price = isset($wholesale_price[$key]) ? $wholesale_price[$key] : null;
    ...
    public function update(Request $request)
    {
        ...
        $input = $request->except(['image', 'product_type', 'files', 'sizes', 'colors', 'purchase_prices', 'old_prices', 'new_prices', 'wholesale_prices', 'stocks', 'images', 'up_id', 'up_sizes', 'up_colors', 'up_purchase_prices', 'up_old_prices', 'up_new_prices', 'up_wholesale_prices', 'up_stocks', 'up_images', 'pro_barcodes', 'up_pro_barcodes']);
