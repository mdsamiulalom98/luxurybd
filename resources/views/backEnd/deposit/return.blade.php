        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box">
                    <h4 class="page-title">Deposit Manage</h4>
                </div>
            </div>
        </div>
        <!-- end page title -->
                            ...
                            <table id="datatable-buttons" class="table table-striped dt-responsive nowrap w-100">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Amount</th>
                                        <th>Method</th>
                                        <th>Sender Number & Transaction ID, Message</th>
                                        <th>Status</th>
                                        <th>Action</th>
                                    </tr>
                                </thead>
                                ...
                                <tbody>
                                    @foreach ($show_data as $key => $value)
                                        <tr>
                                            ...
                                            <td>{{ $value->sender_number }}<br>{{ $value->transaction_id }} <br>
                                                {{ $value->message }}</td>
                                            <td>
                                            ...
                                            <td>
                                                <div class="button-list">
                                                    @if ($value->status == 'pending')
                                                        <form method="post"
                                                            action="{{ route('deposit.status', ['status' => 'accepted']) }}"
                                                            class="d-inline">
                                                            @csrf
                                                        ...
                                                        <form method="post"
                                                            action="{{ route('deposit.status', ['status' => 'cancelled']) }}"
                                                            class="d-inline">
                                                            @csrf
                                                            <input type="hidden" value="{{ $value->id }}"
                                                                name="hidden_id">
                                                    ...
                                                    @if ($value->status == 'accepted')
                                                        <form method="post"
                                                            action="{{ route('deposit.status', ['status' => 'returned']) }}"
                                                            class="d-inline">
                                                            @csrf
                                                            <input type="hidden" value="{{ $value->id }}"
                                                                name="hidden_id">
